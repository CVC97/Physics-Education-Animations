...
class MagneticField(Mobject):
	def __init__(self, center = np.array([0, 0, 0]), list_of_conductors = [(1, np.array([0, 0, 0]))], **kwargs):
		super().__init__(**kwargs)
		self.list_of_conductors = list_of_conductors
	...
	def get_conductor(self, radius = 0.2):
		conductor_list = []
		for conductor in self.list_of_conductors:
			I = conductor[0]
			conductor_position = conductor[1]
			bg_circle = Circle(radius = radius, color = inverted_main_color, stroke_opacity = 0.75, fill_color = inverted_main_color, fill_opacity = 0.75).move_to(self.npla.c2p(*conductor_position))
			bg_circle.z_index = 0.5
			if I > 0:   
				conductor_circle = Circle(radius = radius, stroke_opacity = 0, fill_color =main_color, fill_opacity = 1).move_to(self.npla.c2p(*conductor_position))
				conductor_dir = Dot(radius = 0.05, color = inverted_main_color).move_to(self.npla.c2p(*conductor_position))
			elif I < 0:
				conductor_circle = Circle(radius = radius, stroke_opacity = 0, fill_color = main_color, fill_opacity = 1).move_to(self.npla.c2p(*conductor_position))
				conductor_dir = Cross(scale_factor = 0.1, stroke_color = inverted_main_color, stroke_width = 4).move_to(self.npla.c2p(*conductor_position))
				conductor_dir[1].z_index = 2
			conductor_circle.z_index = 1
			conductor_dir[0].z_index = 2
			single_conductor = VGroup(bg_circle, conductor_circle, conductor_dir)   
			conductor_list.append(single_conductor)
	...
	def get_vector(self, position):
		...
		for conductor in self.list_of_conductors:
			I = conductor[0]
			pos_in_coord = self.npla.c2p(*conductor[1])
			x_i = pos_in_coord[0]
			y_i = pos_in_coord[1]
			field_vector_length = max([np.sqrt(((x-x_i) * self.x_scale)**2 + ((y-y_i) * self.y_scale)**2 + smoothing_factor), 0.5])
			field_vector += I / field_vector_length**3 * ((x-x_i)*UP + (y-y_i)*LEFT)
		return field_vector
